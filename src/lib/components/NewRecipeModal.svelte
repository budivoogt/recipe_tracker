	...
	<form on:submit|preventDefault={handleSubmit}>
		<div class="mb-4 grid gap-4 sm:grid-cols-2">
			...
			<div class="col-span-2">
				<Label for="servingSize" class="mb-2"
					...
					<Range
						...
						min="1"
						max="8"
						bind:value={$servingSizeValue}
						on:change={(e) => updateServingSize($servingSizeValue)}
						class="my-2"
					/>
					<span class=" font-light italic"
						>Serves {$servingSizeValue}
						{$servingSizeValue === 1 ? `person` : `people`}</span
					>
				</Label>
			</div>
			<div class="col-span-2 gap-2">
				...
				{/each}
				<Button on:click={() => addIngredient()} class="mt-2" size="sm"
